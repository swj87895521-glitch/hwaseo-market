...
import { LoyaltyWidget } from '@/components/stores/LoyaltyWidget';
...
import { getStoreLoyaltyBySlug } from '@/mocks/storeLoyalty';
...
export default function StoreDetailPage({
  ...
  const loyalty = mockViewer.isLoggedIn ? getStoreLoyaltyBySlug(params.slug) : null;
              ...
              <StoreInfoPanel title="시장 이용 안내" className="is-guide">
                <div className="store-detail-guide">
                  ...
                  <div className="store-detail-guide__actions">
                    ...
                    <Link href="/stores" className="store-detail-guide__button is-secondary">
                      메인으로 돌아가기
                    </Link>
                  </div>
                </div>
              ...
              {store.featuredProducts.map((product) => (
                <article key={product.id} className="store-featured-card">
                  ...
                  <div className="store-featured-card__body">
                    ...
                    <h3 className="store-featured-card__title">{product.name}</h3>
                    <p className="store-featured-card__summary">{product.summary}</p>
                  </div>
                </article>
              ))}
            </div>
